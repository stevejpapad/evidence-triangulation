...
def prettify_response(response):
    """
    Attempts to extract and prettify a JSON structure from a language model response.
    """

    # If already a dict, assume it's valid
    if isinstance(response, dict):
        #print('its a dict!')
        return response

    # If it's a string, try to parse out JSON
